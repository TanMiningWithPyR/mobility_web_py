    ...
    */
	...
	function init()	{
		if (!ns4)
		{
			...
			if (showToday==1)
			{
  			...
  			sHTML1="<span id='spanLeft'	style='border-style:solid;border-width:1;border-color:#3366FF;cursor:pointer' onmouseover='swapImage(\"changeLeft\",\"left2.gif\");this.style.borderColor=\"#88AAFF\";window.status=\""+scrollLeftMessage+"\"' onclick='javascript:decMonth()' onmouseout='clearInterval(intervalID1);swapImage(\"changeLeft\",\"left1.gif\");this.style.borderColor=\"#3366FF\";window.status=\"\"' onmousedown='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"StartDecMonth()\",100)'	onmouseup='clearTimeout(timeoutID1);clearInterval(intervalID1)'>&nbsp<IMG id='changeLeft' SRC='"+imgDir+"left1.gif' width=10 height=11 BORDER=0>&nbsp</span>&nbsp;"
			sHTML1+="<span id='spanRight' style='border-style:solid;border-width:1;border-color:#3366FF;cursor:pointer'	onmouseover='swapImage(\"changeRight\",\"right2.gif\");this.style.borderColor=\"#88AAFF\";window.status=\""+scrollRightMessage+"\"' onmouseout='clearInterval(intervalID1);swapImage(\"changeRight\",\"right1.gif\");this.style.borderColor=\"#3366FF\";window.status=\"\"' onclick='incMonth()' onmousedown='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"StartIncMonth()\",100)'	onmouseup='clearTimeout(timeoutID1);clearInterval(intervalID1)'>&nbsp<IMG id='changeRight' SRC='"+imgDir+"right1.gif'	width=10 height=11 BORDER=0>&nbsp</span>&nbsp"
	...
	function constructYear() {
		...
		if (!yearConstructed) {
			...
			j =	0
			nStartingYear =	parseInt(yearSelected)-3
...
//	function popUpCalendar(ctl,	ctl2, format, top, left) 
    function popUpCalendar(ctl,	ctl2, top, left) 
	{
	if(ctl.className=="readonly")
	{
	    return
	}
	else
...
//				dateFormat=format;
				...
				{
					if(strTrim(send_Date)!="")
					{
				        yearSelected=parseInt(getdatepart(send_Date ,"y"));
                        monthSelected=parseInt(getdatepart(send_Date ,"m")-1);
                        dateSelected=parseInt(getdatepart(send_Date ,"d"));
                        if(isNaN(dateSelected)|| monthSelected==-1 ||isNaN(yearSelected))
                        {
				...
				crossobj.left =	fixedX==-1 ? ctl.offsetLeft + ctl.offsetWidth + leftpos + 15:	fixedX
				//crossobj.top = fixedY==-1 ?	ctl.offsetTop + ctl.offsetHeight + toppos - 50:	fixedY
				crossobj.top = fixedY==-1 ?	window.event.y:	fixedY
